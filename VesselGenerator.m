% Copyright 2012, 2013 Nicolas Pannetier, ClÈment Debacker, Franck Mauconduit, Thomas Christen, Emmanuel Barbier
...
function model = VesselGenerator(Project,Shape,BV,R,R2,n,Inter,verb)
...
%       n           : Number of vessels
%       Rexclude    : Radius arround the vessels for exclusion
%
%   Output parameters:
...
pass=0;             %% Nombre de sph√®res pass√©es
...
while (k <= n && num < n)
    ...
    if collapsed(Project,Shape,R,R2,q,s,p(k,:),rexc(k)),
        p(k,:)=[ax ay].*rand(1,2);
        continue;
    end
    ...
    if p(k,1)+Rvec(k)>ax-rep_marge
        flag=[1 flag];
        p1=[p(k,1)-ax p(k,2)];
        if collapsed(Project,Shape,R,R2,q,s,p1,rexc(k)),
            p(k,1)=Inf; continue; end
    end
    if p(k,2)+Rvec(k)>ay-rep_marge
        flag=[2 flag];
        p2=[p(k,1) p(k,2)-ay];
        if collapsed(Project,Shape,R,R2,q,s,p2,rexc(k)),
            p(k,1)=Inf; continue; end
    end
    ...
    if p(k,1)-Rvec(k)<0+rep_marge
        flag=[4 flag];
        p4=[p(k,1)+ax p(k,2)];
        if collapsed(Project,Shape,R,R2,q,s,p4,rexc(k)),
            p(k,1)=Inf; continue; end
    end
    if p(k,2)-Rvec(k)<0+rep_marge
        flag=[5 flag];
        p5=[p(k,1) p(k,2)+ay];
        if collapsed(Project,Shape,R,R2,q,s,p5,rexc(k)),
            p(k,1)=Inf; continue; end
    end
    ...
    if ~isempty(flag)
        ...
        elseif numel(flag)==2
            ...
            if sum(flag==1)
                p7(1)=p(k,1)-ax;
                if collapsed(Project,Shape,R,R2,q,s,p7,rexc(k)), p(k,1)=Inf; continue; end
            end
            if sum(flag==2)
                p7(2)=p(k,2)-ay;
                if collapsed(Project,Shape,R,R2,q,s,p7,rexc(k)), p(k,1)=Inf; continue; end
            ...
            if collapsed(Project,Shape,R,R2,q,s,q1,rexc(k)), p(k,1)=Inf; continue; end
            if collapsed(Project,Shape,R,R2,q,s,q2,rexc(k)), p(k,1)=Inf; continue; end
        ...
        elseif numel(flag)==3
            ...
            if collapsed(Project,Shape,R,R2,q,s,q12,rexc(k)), p(k,1)=Inf; continue; end
            if collapsed(Project,Shape,R,R2,q,s,q13,rexc(k)), p(k,1)=Inf; continue; end
            if collapsed(Project,Shape,R,R2,q,s,q23,rexc(k)), p(k,1)=Inf; continue; end
            if collapsed(Project,Shape,R,R2,q,s,q123,rexc(k)), p(k,1)=Inf; continue; end
...
%clc
% disp(sprintf('Nombre d''iterations: %d\nNombre de spheres tracees: %d/%d \nLimite d''iterations: %.2e/5e5 \n',count,k-1,n,lim))
% drawnow;
...
function res = collapsed(Project,Shape,R,R2,Cn,rn,C1,r1)
...
if Project == 1
    if Shape == 1
        stretch_dist = sqrt( X.^2*((R/R2)^2) + Y.^2 );  %For the distance between ellipses, use the stretched distance to prevent overlapping RBCs
        
        sumR = r1 + rn';
        
        if min(stretch_dist) < sumR
            res=1;
        elseif min(stretch_dist)>= sumR
            res=0;
        else
            res=[];
        end
...
% Calcul la fraction volumique intra-cellulaire de la distribution
% M√©thode MonteCarlo (calcul long)
inCount=0;
N=0;
fp=Inf;
fn=Inf;
